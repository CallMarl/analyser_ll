...
/*   Updated: 2018/09/21 03:22:09 by                  ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			ft_lltab_initarr()
{
	...
	if (!(ft_utils_initarr(&g_llfirst, g_lllast - g_llpiv, g_llpiv)))
		return (CODE_ERR1);
	y = ft_utils_getnbrule();
	if (!(ft_utils_initarr(&g_llffirst, g_lllast - g_llpiv, y)))
		return (CODE_ERR1);
	if (!(ft_utils_initarr(&g_llffirst, g_lllast - g_llpiv, y)))
		return (CODE_ERR1);
	if (!(ft_utils_alloclltab(&g_lltab, y, g_lllast - g_llpiv)))
		return (CODE_ERR1);
	return (1);
}
